...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const barFolder = `${process.resourcesPath}\\app.asar.unpacked\\node_modules\\screenshot-desktop\\lib\\win32\\`;
    const batFile = `${barFolder}\\screenCapture_1.3.2.bat`;

    exec(`${batFile} ${imgPath} ${displayChoice}`, {
      cwd: barFolder,
      windowspatpHide: true
    }, (err, stdout) => {
      if (err) {
        return reject(err)
      } else {
